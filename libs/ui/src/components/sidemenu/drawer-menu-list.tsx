...
const DrawerMenuList: ComponentType<DrawerProps> = ({
  sideMenuItems,
  setOpenDrawer,
}) => {
    ...
    <Box>
      ...
      >
        <IconButton onClick={() => setOpenDrawer(false)}>
          <Close color="primary" sx={{ fontSize: 24 }} />
        </IconButton>
      </Box>
      <Box sx={{ mt: 6 }}>
        {sideMenuItems.map((menuItem) => {
          ...
          return (
            <Link key={menuItem.key} href={menuItem.url} color="inherit">
              <MenuItem
                sx={{
                  display: 'flex',
                  alignItems: 'center',
              ...
              >
                ...
                {menuItem.info && (
                  <Chip label={menuItem.info} color="primary" />
                )}
              </MenuItem>
            </Link>
          )
