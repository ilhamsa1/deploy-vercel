...
BEGIN
    ...
    -- If the new row status is 'requires_payment_method', process it
...
CREATE OR REPLACE TRIGGER on_allocate_payment_trigger
AFTER INSERT OR UPDATE ON public.payment_intent
FOR EACH ROW
WHEN (NEW.status = 'requires_payment_method')
